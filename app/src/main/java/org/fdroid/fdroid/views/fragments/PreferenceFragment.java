...
public class PreferenceFragment
     ...
     * when the user grants root access for K-App.
     */
    ...
    /**
     * Initializes SystemInstaller preference, which can only be enabled when K-App is installed as a system-app
     */
            ...
            public boolean onPreferenceClick(Preference preference) {
                ...
                if (pref.isChecked()) {
                    if (Installer.hasSystemPermissions(getActivity(), getActivity().getPackageManager())) {
                        // system-permission are granted, i.e. K-App is a system-app
                        SharedPreferences.Editor editor = pref.getSharedPreferences().edit();
